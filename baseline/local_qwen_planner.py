...
class LocalQwenPlanner(Model):
    def __init__(self, cfg, run):
        ...
        event(self.run, 'local_qwen_loaded', model_object_id=id(self.model), load_count=1,
              ...
              backend='transformers', reasoning=True)
    ...
    def generate(self, messages, tools=None):
        ...
        deadline = start + self.cfg['timeout']
        ...
        try:
            ...
            budget = min(self.cfg['max_generation_tokens'],
                         self.cfg['context_length'] - n - self.cfg['context_margin'])
            record.update(input_tokens=n, effective_generation_budget=budget)
            if budget <= 0:
                raise RuntimeError('PLANNER_DELIBERATION: context limit reached')
            ...
            record.update(output_tokens=len(tokens), raw_response=raw,
                eos_token_ids=eos, final_token_id=int(tokens[-1]),
                generation_limit_hit=len(tokens) >= budget, timeout=time.monotonic() >= deadline,
                peak_allocated_bytes=torch.cuda.max_memory_allocated())
            if record['timeout']:
                raise TimeoutError('TIMING / PLANNER_DELIBERATION: local generation exceeded timeout')
            if record['generation_limit_hit']:
                raise RuntimeError('PLANNER_DELIBERATION: generation limit reached')
            if tools:
                try:
            ...
            record['time_to_valid_decision'] = time.monotonic() - start
